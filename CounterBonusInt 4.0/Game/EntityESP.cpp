...
EntityESP::EntityESP(C_CSPlayer* pEntity) : m_pEntity(pEntity)
{}
...
void EntityESP::RenderESP(DrawManager& renderer)
{
	...
	if (bIsEnemy && Options::g_bESPEnemyHitboxesEnabled || !bIsEnemy && Options::g_bESPAllyHitboxesEnabled)
	{
		...
		auto vHead = Toolbox::getInstance()->GetEntityBone(m_pEntity, ECSPlayerBones::head_0);

		//Offset the head a bit, so the box looks nicer
		vHead.z += 15.0f;

		Vector vScreenOrigin, vScreenHead;
		if (Toolbox::getInstance()->WorldToScreen(vHead, vScreenHead) && Toolbox::getInstance()->WorldToScreen(vOrigin, vScreenOrigin))
		{
...
void EntityESP::RenderName(DrawManager& renderer)
{
	...
	if (bIsEnemy && Options::g_bESPEnemyNamesEnabled || !bIsEnemy && Options::g_bESPAllyNamesEnabled)
	{
		auto allyColor = Options::g_bESPAllyColor;
		auto enemyColor = Options::g_bESPEnemyColor;
		...
		if (Toolbox::getInstance()->WorldToScreen(vOrigin, vScreenOrigin))
		{
			player_info_t pInfo;
