...
namespace SourceEngine
{
    ...
    CGlobalVarsBase* Interfaces::GlobalVars()
    {
        if(!m_pGlobals)
		{
            auto uAddress = Toolbox::getInstance()->FindSignature(XorStr("client.dll"), XorStr("A1 ? ? ? ? 5F 8B 40 10"));
    ...
    IClientMode* Interfaces::ClientMode()
    {
        if(!m_pClientMode)
		{
            auto uAddress = *(DWORD*)(Toolbox::getInstance()->FindSignature(XorStr("client.dll"), XorStr("8B 35 ? ? ? ? 85 FF 74 73")) + 2);
