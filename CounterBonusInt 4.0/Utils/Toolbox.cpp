...
SourceEngine::Vector Toolbox::GetEntityBone(SourceEngine::IClientEntity* pEntity, SourceEngine::ECSPlayerBones Bone)
{
...
bool Toolbox::ScreenTransform(const SourceEngine::Vector& point, SourceEngine::Vector& screen)
{
...
bool Toolbox::WorldToScreen(const SourceEngine::Vector &origin, SourceEngine::Vector &screen)
{
...
uint64_t Toolbox::FindSignature(const char* szModule, const char* szSignature)
{
	//CREDITS: learn_more
...
bool Toolbox::Clamp(SourceEngine::QAngle &angles)
{
...
void Toolbox::Normalize(SourceEngine::QAngle& angle)
{
	while (angle.x > 89.0f) {
		angle.x -= 180.f;
	}
...
void Toolbox::ClampAngles(SourceEngine::QAngle &angles)
{
	if (angles.y > 180.0f)
		angles.y = 180.0f;
